...
class MarkOnMapViewController: UIViewController {
    ...
    @IBAction func btnOk_Clicked(_ sender: AnyObject) {
        ...
        if isFromLocation {
            delegate?.didFinishSelectFromLocation(mapView.screen(toLocation: mapView.center))
            
        } else {
            delegate?.didFinishSelectToLocation(mapView.screen(toLocation: mapView.center))
        }
